...
class IdentityRoutes
{
    ...
    private function validatePayload(array $payload, bool $requireName): ?WP_REST_Response
    {
        ...
        if ($requireName) {
            $validator->required('name', 'Organisation name', $payload);
        }
        $validator->allowedValue('type', 'Organisation type', ['company', 'club', 'association', 'public_body', 'partner'], $payload);
        if (isset($payload['status'])) {
            $validator->allowedValue('status', 'Organisation status', ['active', 'archived'], $payload);
        }
