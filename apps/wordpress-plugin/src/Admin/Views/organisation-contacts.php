...
<div class="wrap dbg-platform-admin">
    ...
    <?php else : ?>
        ...
        <div class="dbg-platform-panel">
            <h2>Contacts</h2><p><?php echo esc_html($result['pagination']['total']); ?> contact(s)</p>
            <table class="widefat striped">
                <thead><tr><th>ID</th><th>Contact</th><th>Created</th><th>Updated</th><th>Save</th><th>Actions</th></tr></thead><tbody>
                <?php if (empty($contacts)) : ?><tr><td colspan="6">No contacts found.</td></tr><?php else : foreach ($contacts as $contact) : ?>
                    <tr><td><?php echo esc_html($contact['id']); ?></td><td>
                        ...
                        <input type="text" name="job_title" value="<?php echo esc_attr($contact['job_title']); ?>"> <input type="text" name="department" value="<?php echo esc_attr($contact['department']); ?>">
                        <br><input type="email" name="email" value="<?php echo esc_attr($contact['email']); ?>"> <input type="text" name="phone" value="<?php echo esc_attr($contact['phone']); ?>"> <input type="text" name="mobile" value="<?php echo esc_attr($contact['mobile']); ?>">
