...
$filters = [
    ...
    'is_favorite' => isset($_GET['is_favorite']) ? sanitize_text_field($_GET['is_favorite']) : '',
...
<div class="wrap dbg-platform-admin">
    <h1>Media</h1>
    <p>Upload, filter, favorites, metadata, rename, folder, version, tags, bulk manage, sort and review files linked to DBG Platform assets.</p>
    ...
    <div class="dbg-platform-panel">
        ...
        <?php if (!empty($tags)) : ?><p><?php foreach ($tags as $tag) : ?><span class="dbg-media-tag" style="background:<?php echo esc_attr($tag['color'] ?: '#f0f0f1'); ?>"><?php echo esc_html($tag['name']); ?></span><?php endforeach; ?></p><?php endif; ?>
    </div>
    ...
    <div class="dbg-platform-panel">
        ...
        <form method="get">
            ...
            <input type="text" name="meta_key" placeholder="Metadata key" value="<?php echo esc_attr($filters['meta_key']); ?>">
            <input type="text" name="meta_value" placeholder="Metadata value" value="<?php echo esc_attr($filters['meta_value']); ?>">
            ...
            <select name="sort_by"><option value="id" <?php selected($sort['sort_by'], 'id'); ?>>Sort by ID</option><option value="name" <?php selected($sort['sort_by'], 'name'); ?>>Sort by name</option><option value="type" <?php selected($sort['sort_by'], 'type'); ?>>Sort by type</option><option value="size" <?php selected($sort['sort_by'], 'size'); ?>>Sort by size</option><option value="favorite" <?php selected($sort['sort_by'], 'favorite'); ?>>Sort by favorite</option><option value="status" <?php selected($sort['sort_by'], 'status'); ?>>Sort by status</option><option value="created_at" <?php selected($sort['sort_by'], 'created_at'); ?>>Sort by created</option><option value="updated_at" <?php selected($sort['sort_by'], 'updated_at'); ?>>Sort by updated</option></select>
    ...
    <div class="dbg-platform-panel">
        <h2>File records</h2>
        <p><?php echo esc_html($pagination['total']); ?> file(s) · page <?php echo esc_html($pagination['page']); ?> / <?php echo esc_html($pagination['total_pages']); ?> · sorted by <?php echo esc_html($sort['sort_by']); ?> <?php echo esc_html($sort['sort_order']); ?><?php if ($filters['meta_key'] || $filters['meta_value']) : ?> · metadata filter active<?php endif; ?></p>
        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
            ...
            <table class="widefat striped">
                <thead><tr><th><input type="checkbox" onclick="document.querySelectorAll('.dbg-file-select').forEach(cb => cb.checked = this.checked);"></th><th><a href="<?php echo $sortLink('id'); ?>">ID</a></th><th><a href="<?php echo $sortLink('favorite'); ?>">★</a></th><th>Preview</th><th><a href="<?php echo $sortLink('name'); ?>">Name</a></th><th>Tags</th><th>Metadata</th><th>Rename</th><th>Folder</th><th><a href="<?php echo $sortLink('type'); ?>">Type</a></th><th><a href="<?php echo $sortLink('size'); ?>">Size</a></th><th><a href="<?php echo $sortLink('status'); ?>">Status</a></th><th>Download</th><th>Move</th><th>New version</th><th>Versions</th><th>Archive</th></tr></thead>
                <tbody>
                <?php if (empty($files)) : ?><tr><td colspan="17">No file records found.</td></tr><?php else : ?>
                    <?php foreach ($files as $file) : ?>
                        <?php $downloadUrl = wp_nonce_url(admin_url('admin-post.php?action=dbg_download_file&file_id=' . absint($file['id'])), 'dbg_download_file'); $versions = $versionRepository->allForFile((int) $file['id']); $fileTags = $tagRepository->tagsForFile((int) $file['id']); $fileTagIds = array_map('intval', array_column($fileTags, 'id')); $isFavorite = !empty($file['is_favorite']); $metadata = $metadataRepository->allForFile((int) $file['id']); ?>
                        <tr>
                            ...
                            <td><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><input type="hidden" name="action" value="dbg_toggle_file_favorite"><input type="hidden" name="file_id" value="<?php echo esc_attr($file['id']); ?>"><input type="hidden" name="is_favorite" value="<?php echo $isFavorite ? '0' : '1'; ?>"><?php wp_nonce_field('dbg_toggle_file_favorite'); ?><button class="button" title="Favorite"><?php echo $isFavorite ? '★' : '☆'; ?></button></form></td>
                            ...
                            <td><?php if (empty($metadata)) : ?>—<?php else : ?><?php foreach (array_slice($metadata, 0, 3) as $key => $value) : ?><div><strong><?php echo esc_html($key); ?></strong>: <?php echo esc_html(is_scalar($value) ? (string) $value : wp_json_encode($value)); ?></div><?php endforeach; ?><?php endif; ?><a class="button" href="<?php echo esc_url(admin_url('admin.php?page=dbg-platform-media-metadata&file_id=' . absint($file['id']))); ?>">Edit</a></td>
