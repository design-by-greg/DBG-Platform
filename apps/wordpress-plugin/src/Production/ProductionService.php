...
class ProductionService
{
    ...
    public function archive(int $id): bool
    {
        ...
        $this->audit->record('archived', 'production_job', $id, ['before' => $before, 'after' => $after, 'archived' => $done]);
        return $done;
    }
    ...
    public function restore(int $id): bool
    {
        ...
        $this->audit->record('restored', 'production_job', $id, ['before' => $before, 'after' => $after, 'restored' => $done]);
        return $done;
    }
    ...
    public function changeStatus(int $id, string $status): bool
    {
        ...
        if ($status === 'in_progress' && empty($before['started_at'])) { $payload['started_at'] = current_time('mysql'); }
        ...
        $this->audit->record('status_changed', 'production_job', $id, ['before' => $before, 'after' => $after, 'status' => $status, 'updated' => $done]);
        return $done;
    }
    ...
    public function validationErrors(array $data, bool $create, ?int $jobId = null): array
    {
        ...
        if (isset($merged['title']) && strlen((string) $merged['title']) > 255) { $errors[] = 'Production title must be 255 characters or less.'; }
        ...
        if (!empty($merged['planned_start_at']) && !empty($merged['planned_end_at']) && strtotime((string) $merged['planned_end_at']) < strtotime((string) $merged['planned_start_at'])) { $errors[] = 'Planned end date cannot be before planned start date.'; }
