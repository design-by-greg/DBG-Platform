...
class FileRecordRepository
{
    private array $sortableColumns = [
        'id' => 'id', 'name' => 'original_name', 'type' => 'mime_type', 'size' => 'size', 'favorite' => 'is_favorite', 'status' => 'status', 'created_at' => 'created_at', 'updated_at' => 'updated_at',
    ];
    ...
    public function paginated(array $filters = [], int $page = 1, int $perPage = 25): array
    {
        ...
        return ['items' => $wpdb->get_results($wpdb->prepare($sql, $queryParams), ARRAY_A) ?: [], 'pagination' => ['page' => $page, 'per_page' => $perPage, 'total' => $total, 'total_pages' => max(1, (int) ceil($total / $perPage))], 'sort' => $this->normaliseSort($filters)];
    }
    ...
    public function duplicateGroups(): array
    {
        ...
        foreach ($groups as &$group) { $group['files'] = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$table} WHERE file_hash = %s AND status != 'archived' ORDER BY id DESC", $group['file_hash']), ARRAY_A) ?: []; }
        return $groups;
    }
    ...
    private function whereSql(array $filters): array
    {
        ...
        if (!empty($filters['meta_key']) && !empty($filters['meta_value'])) { array_pop($where); array_pop($where); array_pop($params); array_pop($params); $where[] = "id IN (SELECT file_record_id FROM {$metadataTable} WHERE meta_key = %s AND meta_value LIKE %s)"; $params[] = sanitize_key($filters['meta_key']); $params[] = '%' . $wpdb->esc_like(sanitize_text_field($filters['meta_value'])) . '%'; }
        ...
        if (!empty($filters['tag_ids']) && is_array($filters['tag_ids'])) { $tagIds = array_values(array_unique(array_filter(array_map('absint', $filters['tag_ids'])))); if (!empty($tagIds)) { $where[] = "id IN (SELECT file_record_id FROM {$wpdb->prefix}dbg_file_tag_map WHERE tag_id IN (" . implode(',', array_fill(0, count($tagIds), '%d')) . "))"; foreach ($tagIds as $tagId) { $params[] = $tagId; } } }
    ...
    private function orderSql(array $filters): string { $sort = $this->normaliseSort($filters); return ' ORDER BY ' . $this->sortableColumns[$sort['sort_by']] . ' ' . $sort['sort_order']; }
    private function normaliseSort(array $filters): array { $sortBy = sanitize_key($filters['sort_by'] ?? 'id'); $sortOrder = strtoupper(sanitize_key($filters['sort_order'] ?? 'DESC')); if (!isset($this->sortableColumns[$sortBy])) { $sortBy = 'id'; } if (!in_array($sortOrder, ['ASC', 'DESC'], true)) { $sortOrder = 'DESC'; } return ['sort_by' => $sortBy, 'sort_order' => $sortOrder]; }
    public function find(int $id): ?array { global $wpdb; $row = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}dbg_file_records WHERE id = %d", $id), ARRAY_A); return $row ?: null; }
    ...
    public function replace(int $id, array $data): bool { global $wpdb; return false !== $wpdb->update($wpdb->prefix . 'dbg_file_records', ['original_name' => sanitize_file_name($data['original_name'] ?? ''), 'filename' => sanitize_file_name($data['filename'] ?? ''), 'mime_type' => sanitize_text_field($data['mime_type'] ?? ''), 'size' => absint($data['size'] ?? 0), 'file_hash' => sanitize_text_field($data['file_hash'] ?? ''), 'path' => sanitize_text_field($data['path'] ?? ''), 'url' => esc_url_raw($data['url'] ?? ''), 'thumbnail_path' => sanitize_text_field($data['thumbnail_path'] ?? ''), 'thumbnail_url' => esc_url_raw($data['thumbnail_url'] ?? ''), 'status' => 'active', 'updated_at' => current_time('mysql')], ['id' => $id]); }
    ...
    public function restore(int $id): bool { global $wpdb; return false !== $wpdb->update($wpdb->prefix . 'dbg_file_records', ['status' => 'active', 'updated_at' => current_time('mysql')], ['id' => $id]); }
    ...
    public function bulkRestore(array $ids): int { $count = 0; foreach ($this->cleanIds($ids) as $id) { $count += $this->restore($id) ? 1 : 0; } return $count; }
