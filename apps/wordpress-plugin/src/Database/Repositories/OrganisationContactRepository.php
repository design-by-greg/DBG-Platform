...
class OrganisationContactRepository
{
    ...
    public function create(array $data): int
    {
        ...
        if ($isPrimary) {
            $this->clearPrimary($organisationId);
        }
    ...
    {
        return $this->update($id, ['status' => 'archived', 'is_primary' => 0]) && $this->setArchivedAt($id, current_time('mysql'));
    }
    ...
    private function whereSql(array $filters): array
    {
        ...
        if (!empty($filters['department'])) { $where[] = 'department LIKE %s'; $params[] = '%' . $wpdb->esc_like(sanitize_text_field($filters['department'])) . '%'; }
        if (!empty($filters['search'])) {
            $where[] = '(first_name LIKE %s OR last_name LIKE %s OR email LIKE %s OR job_title LIKE %s OR department LIKE %s)';
